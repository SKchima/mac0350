...
def buscar_alunos_com_filtro(busca: str = '', minimo: int = 0, maximo: int = 5):
    with Session(engine) as session:
        query = select(Aluno).where(col(Aluno.nome).contains(busca)).order_by(Aluno.nome).offset(minimo).limit(maximo + 1)
        registros = session.exec(query).all()
        
        tem_proxima = len(registros) > maximo
        return registros[:maximo], tem_proxima
...
def lista(request: Request, busca: str | None = '', pagina: int = 1):
    ...
    alunos, tem_proxima = buscar_alunos_com_filtro(busca, minimo, maximo)
...
def editarAlunos(request: Request):
    return templates.TemplateResponse(request=request, name="options.html")
